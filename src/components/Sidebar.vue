...
const onChatShowMore = (chat: Chat) => {
  ...
  const parent = document.getElementsByClassName("chat_info_pos"+chat.id)[0]
  if (!parent)
    return
...
const onChatCloseMore = (chat: Chat) => {
  /// console.log(JSON.stringify(chat))
  chat.showMore = false
}
...
<template>
  ...
  <aside class="flex">
    <div
      ...
      <div class="mx-2 mb-2">
        <button v-if="isConnected" @click="onNewChat"
      ...
      <div class="h-full space-y-4 overflow-y-auto border-b border-gray-200 px-2 py-4 dark:border-gray-800">
        <button v-for="chat in sortedChats" @click="onSwitchChat(chat.id!)" :class="{
          ...
          <div style="display: flex;justify-content: space-between">
            ...
            <div style="position: relative" class="chat_info_pos" 
            :class="'chat_info_pos' + chat.id">
              ...
              <!-- 隐藏的弹出菜单 -->
              <div @click="onChatCloseMore(chat!)" id="chatMenu" style="
  ...
  align-items: center;
  z-index: 1000" v-show="chat.showMore">
